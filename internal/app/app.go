...
func Create() *App {
	...
	a.dirData = a.checkDirData("assets")
...
func (a *App) checkDirData(dirDataName string) string {
	...
	path = filepath.Join(goPath, "src", "github.com", "inux", "universe", dirDataName)
	// Checks data path
	if _, err := os.Stat(path); err == nil {
		return path
	}
	...
	for _, j := range paths {
		// Checks data path
		path = filepath.Join(j, "src", "github.com", "inux", "universe", dirDataName)
		if _, err := os.Stat(path); err == nil {
			return path
		}
	}
